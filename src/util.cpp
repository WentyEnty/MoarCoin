...
int nAnonymizeMoarCoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MoarCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MoarCoin
    // Mac: ~/Library/Application Support/MoarCoin
    // Unix: ~/.MoarCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MoarCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MoarCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MoarCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong MoarCoin will not work properly.");
