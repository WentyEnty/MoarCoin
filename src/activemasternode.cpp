...
//
// Bootup the masternode, look for a 10000 MOAR input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in MoarCoin.conf */
...
{
    CMoarCoincoinAddress address(collateralAddress);
