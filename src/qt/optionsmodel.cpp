...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMoarCoinAmount"))
        settings.setValue("nAnonymizeMoarCoinAmount", 1000);
    nAnonymizeMoarCoinAmount = settings.value("nAnonymizeMoarCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMoarCoinAmount"))
        SoftSetArg("-anonymizeMoarCoinamount", settings.value("nAnonymizeMoarCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMoarCoinAmount:
            return QVariant(nAnonymizeMoarCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMoarCoinAmount:
            nAnonymizeMoarCoinAmount = value.toInt();
            settings.setValue("nAnonymizeMoarCoinAmount", nAnonymizeMoarCoinAmount);
            emit AnonymizeMoarCoinAmountChanged(nAnonymizeMoarCoinAmount);
            break;
        default:
            break;
        }
