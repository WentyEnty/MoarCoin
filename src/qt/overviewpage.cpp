...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMoarCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMoarCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMoarCoinAmount = strAnonymizeMoarCoinAmount.remove(strAnonymizeMoarCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMoarCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMoarCoinAmount*COIN) nMaxToAnonymize = nAnonymizeMoarCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMoarCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMoarCoinAmount));
        strAnonymizeMoarCoinAmount = strAnonymizeMoarCoinAmount.remove(strAnonymizeMoarCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMoarCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
