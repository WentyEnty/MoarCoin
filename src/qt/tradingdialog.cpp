    ...
    model(0)
{
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "MOAR(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "MOAR(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(MOAR)"<<"TOTAL COST(BTC");
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://future-exchange.com/api/v1.1/public/GetMarketSummary?market=btc-MOAR");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://future-exchange.com/api/v1.1/public/getorderbook?market=BTC-MOAR&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://future-exchange.com/api/v1.1/public/getmarkethistory?market=BTC-MOAR&count=100");
      return Response;
}
...
QString tradingDialog::BuyTX(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-MOAR&quantity=";
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://future-exchange.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-MOAR";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=MOAR";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-MOAR&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>MOAR Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> MOAR");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->TXSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(TXSupply,'i',8) + "</span><b> MOAR</b>");
    ...
    ui->TXDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(TXDemand,'i',8) + "</span><b> MOAR</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("MOAR");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->TXBalanceLabel,*ui->TXAvailableLabel_2,*ui->TXPendingLabel, QString::fromUtf8("MOAR"),Response);
                    }
                break;
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("MOAR");
...
void tradingDialog::on_Sell_Max_Amount_clicked()
{
    ...
    QString responseA = GetBalance("MOAR");
    QString str;
