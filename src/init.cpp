...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MoarCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: MoarCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMoarCoinamount=<n> " + _("Keep N MoarCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MoarCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MoarCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MoarCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MoarCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MoarCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MoarCoin to complete") << "\n";
    ...
    nAnonymizeMoarCoinAmount = GetArg("-anonymizeMoarCoinamount", 0);
    if(nAnonymizeMoarCoinAmount > 999999) nAnonymizeMoarCoinAmount = 999999;
    if(nAnonymizeMoarCoinAmount < 2) nAnonymizeMoarCoinAmount = 2;
    ...
    LogPrintf("Anonymize MoarCoin Amount %d\n", nAnonymizeMoarCoinAmount);
