...
CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        CMoarCoincoinAddress address(ks);
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CMoarCoincoinAddress address(innerID);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new MoarCoin address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"MoarCoinaddress\"    (string) The new MoarCoin address\n"
            "\nExamples:\n"
    ...
    return CMoarCoincoinAddress(keyID).ToString();
}


CMoarCoincoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CMoarCoincoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current MoarCoin address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"MoarCoinaddress\"   (string) The account MoarCoin address\n"
            "\nExamples:\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"MoarCoinaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"MoarCoinaddress\"  (string, required) The MoarCoin address to be associated with an account.\n"
            "2. \"account\"         (string, required) The account to assign the address to.\n"
            "\nExamples:\n"
            + HelpExampleCli("setaccount", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\" \"tabby\"")
            + HelpExampleRpc("setaccount", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\", \"tabby\"")
        );

    CMoarCoincoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MoarCoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"MoarCoinaddress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"MoarCoinaddress\"  (string, required) The MoarCoin address for account lookup.\n"
            "\nResult:\n"
            "\"accountname\"        (string) the account address\n"
            "\nExamples:\n"
            + HelpExampleCli("getaccount", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\"")
            + HelpExampleRpc("getaccount", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\"")
        );
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"MoarCoinaddress\"  (string) a MoarCoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CMoarCoincoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CMoarCoincoinAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"MoarCoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSent an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
           "1. \"MoarCoinaddress\"  (string, required) The MoarCoin address to send to.\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("sendtoaddress", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\" 0.1")
            + HelpExampleCli("sendtoaddress", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\" 0.1 \"donation\" \"seans outpost\"")
            + HelpExampleRpc("sendtoaddress", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\", 0.1, \"donation\", \"seans outpost\"")
        );
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"MoarCoinaddress\",     (string) The MoarCoin address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CMoarCoincoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CMoarCoincoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CMoarCoincoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"MoarCoinaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"MoarCoinaddress\"  (string, required) The MoarCoin address to use for the private key.\n"
            ...
            + HelpExampleCli("signmessage", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\" \"my message\"") +
            "\nVerify the signature\n"
            + HelpExampleCli("verifymessage", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\" \"signature\" \"my message\"") +
            "\nAs json rpc\n"
            + HelpExampleRpc("signmessage", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\", \"my message\"")
        );
    ...
    CMoarCoincoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"MoarCoinaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given MoarCoinaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"MoarCoinaddress\"  (string, required) The MoarCoin address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (numeric) The total amount inBHDreceived at this address.\n"
            "\nExamples:\n"
            "\nThe amount from transactions with at least 1 confirmation\n"
            + HelpExampleCli("getreceivedbyaddress", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\"") +
            "\nThe amount including unconfirmed transactions, zero confirmations\n"
            + HelpExampleCli("getreceivedbyaddress", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\" 0") +
            "\nThe amount with at least 10 confirmation, very safe\n"
            + HelpExampleCli("getreceivedbyaddress", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\" 10") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("getreceivedbyaddress", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\", 10")
       );

    // Bitcoin address
    CMoarCoincoinAddress address = CMoarCoincoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MoarCoin address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toMoarCoinaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a MoarCoin address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toMoarCoinaddress\"  (string, required) The MoarCoin address to send funds to.\n"
            "3. amount                (numeric, required) The amount in MOAR. (transaction fee is added on top).\n"
            ...
            + HelpExampleCli("sendfrom", "\"\" \"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\" 0.01") +
            "\nSend 0.01 from the tabby account to the given address, funds must have at least 10 confirmations\n"
            + HelpExampleCli("sendfrom", "\"tabby\" \"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\" 0.01 10 \"donation\" \"seans outpost\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("sendfrom", "\"tabby\", \"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\", 0.01, 10, \"donation\", \"seans outpost\"")
        );

    EnsureWalletIsUnlocked();

    string strAccount = AccountFromValue(params[0]);
    CMoarCoincoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MoarCoin address");
    CAmount nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The MoarCoin address is the key, the numeric amount inBHDis the value\n"
            "      ,...\n"
            "    }\n"
            ...
            + HelpExampleCli("sendmany", "\"tabby\" \"{\\\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\\\":0.01,\\\"MqXy5e1QhqYgUqXn8mWqhn1dogj2caf7hb\\\":0.02}\"") +
            "\nSend two amounts to two different addresses setting the confirmation and comment:\n"
            + HelpExampleCli("sendmany", "\"tabby\" \"{\\\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\\\":0.01,\\\"MqXy5e1QhqYgUqXn8mWqhn1dogj2caf7hb\\\":0.02}\" 10 \"testing\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("sendmany", "\"tabby\", \"{\\\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\\\":0.01,\\\"MqXy5e1QhqYgUqXn8mWqhn1dogj2caf7hb\\\":0.02}\", 10, \"testing\"")
        );
    ...
    set<CMoarCoincoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CMoarCoincoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid MoarCoin address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a MoarCoin address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of MoarCoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) MoarCoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"MoarCoinaddress\"  (string) A MoarCoin address associated with the keys.\n"

            "\nExamples:\n"
            "\nAdd a multisig address from 2 addresses\n"
            + HelpExampleCli("addmultisigaddress", "2 \"[\\\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\\\",\\\"MqXy5e1QhqYgUqXn8mWqhn1dogj2caf7hb\\\"]\"") +
            "\nAs json rpc call\n"
            + HelpExampleRpc("addmultisigaddress", "2, \"[\\\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\\\",\\\"MqXy5e1QhqYgUqXn8mWqhn1dogj2caf7hb\\\"]\"")
        ;
        throw runtime_error(msg);
    }
    ...
    return CMoarCoincoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CMoarCoincoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CMoarCoincoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CMoarCoincoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"MoarCoinaddress\",    (string) The MoarCoin address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in MOAR. This is negative for the 'send' category, and for the\n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
       throw runtime_error(
            ...
            "    \"address\":\"MoarCoinaddress\",    (string) The MoarCoin address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in MOAR. This is negative for the 'send' category, and for the 'move' category for moves \n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"MoarCoinaddress\",   (string) The MoarCoin address involved in the transaction\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending MOAR\n"
            "\nArguments:\n"
...
Value walletlock(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 0))
        throw runtime_error(
            ...
            + HelpExampleCli("sendtoaddress", "\"M5tXqgY4r2RoEdqYk3QsAqFckyf9pRHN6i\" 1.0") +
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending MoarCoin\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"MoarCoinaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; MoarCoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in MOAR/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new MoarCoin stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid MoarCoin stealth address."));
        return result;
    };
